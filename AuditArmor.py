  ...
  --module            |    add custom module (e.g. "google.py")
	...
	if opts.proxy:
		...
		if proxy.startswith("http://"):
			...
			logger.error("This Proxy Not Supported from Audit Armor")
			exit()
		else:
	...
	if opts.ver:
		...
		exit()
	if opts.url != None and opts.rfile != None:
		logger.error("You Can't Start Audit Armor With List option and url option ")
		exit()
	if opts.cookie:
	...
	if opts.module:
		...
		try:
			...
			if name == None or description == None or date == None or emails == None or authors == None:
				logger.error('i need more informations abount module please read the developers docs on Audit Armor website')
				exit()
			email = ''
			author = ''
			for i in emails:
